...
### Estado actualizado
- Queda disponible `npm run qa:facturacion` para validar el flujo funcional de facturación sin dejar datos QA persistidos.
- El script crea un documento temporal, vincula un movimiento financiero libre, valida detalle, evita doble documentación, emite el documento y limpia el documento/configuración temporal.
- Verificación local: `npm run qa:facturacion` PASS; posterior a la limpieza, `documentos_facturacion = 0` y `documentos_pagos = 0`.

### Siguiente foco recomendado
1. QA visual en navegador de descarga PDF/RIDE desde `facturacion.html` con un documento real conservado.
2. Transmisiones WebRTC en Render.
3. QA responsive visual con datos reales.
