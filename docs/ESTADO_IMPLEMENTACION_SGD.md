...
| 3.6 Generacion de Fixture | Alto | Generacion por evento, filtros por grupo/jornada/fecha, vista plantilla y exportaciones. Eliminacion de fixture con confirmacion ante partidos finalizados. Regeneracion preservando partidos jugados (round-robin solo sobre pendientes, con continuacion de jornadas). Auto-estado: al programar fecha en un partido el `estado` pasa automáticamente a `'programado'`; al borrarla vuelve a `'pendiente'`; al guardar planilla queda `'finalizado'`. Estados terminales no se tocan. |
...
14. Portal público / plantillas de exportación (pendiente):
